...
따릉이의 문제는 수요 불균형이다. 특정 대여소는 자전거가 고갈되고, 다른 대여소에는 쌓인다. 재배치 트럭은 하루에 수백 번 이동하지만 데이터 기반 기준이 없으면 항상 뒤늦은 대응이 된다.

2025년 Q4 대여 데이터 8,559,939건으로 세 가지 질문에 답했다: 누가 이용하는가, 언제·어디서 수요 급증이 반복되는가, 얼마나 필요한지 사전에 예측할 수 있는가.
...
K-Means와 GMM을 k=2~6 범위에서 Silhouette, Calinski-Harabasz, Davies-Bouldin, Stability ARI 4개 지표로 비교했다. 두 모델 모두 k=3에서 수렴했다. Silhouette Score 0.185는 낮지만 이용시간·거리가 연속 분포라 군집 경계가 원래 모호하다. Stability ARI 0.999로 군집 구조 자체는 안정적이다.

초기에는 `is_weekend`와 `bike_type`이 군집 분리를 지나치게 주도해 제외했다. 시간·요일을 주기형 인코딩으로 바꾼 뒤 이용 행태 중심의 군집이 형성됐다.
...
Z-score와 IQR은 시계열 전체 통계로 이상을 판단해 매일 반복되는 출근 피크(07~09시)를 이상으로 오탐한다. STL은 일별 계절성을 먼저 제거하고 잔차에서만 이상을 탐지해 구조적 패턴과 실제 이상을 구분한다.
...
- 출퇴근 피크(07~09시, 17~19시)에서 MAE가 높다. 급격한 수요 변화는 lag만으로 완전히 포착하기 어렵고, 날씨·이벤트 정보가 추가 개선 여지가 있다.
- 환승역·업무지구 인근 대여소에서 오차가 크고, 주거지형 대여소는 낮다.
- 잔차는 0 근처에 집중되고 대칭 분포 → 체계적 편향 없음
