...
서울시 따릉이 대여 데이터를 분석하여 자전거 재배치 의사결정을 자동화하기 위한 프로젝트입니다.

| 항목 | 내용 |
|---|---|
| 데이터 | 2025년 Q4 대여 기록 |
| 규모 | 8,559,939건 |
| 분석 순서 | 사용자 클러스터링 → 수요 이상 탐지 → 시간별 수요 예측 |

**목차**

- [01. 사용자 클러스터링](#01-사용자-클러스터링-user-clustering)
- [02. 대여소 수요 이상 탐지](#02-대여소-수요-이상-탐지-demand-anomaly-detection)
- [03. 수요 예측](#03-수요-예측-demand-forecasting)
...
> 단거리 두 군집은 평일 위주, 장거리형은 주말 비중이 유지.
...
상위 50개 대여소의 시간대별 대여량에 STL을 적용해 잔차가 3σ를 초과하는 시점을 이상으로 정의.
period=24로 하루 주기 계절성을 분리하므로 주간 구조 차이(주말 등)는 잔차에 일부 남아 별도 패턴으로 해석.
...
> 주말(45.9%)은 이상이라기보다 구조적 수요 차이다.
> 평일 17~19시 급증(18.0%)이 운영 규칙으로 쓰기 가장 좋은 반복 패턴이다.
...
- **출근 피크 대비 자동화**: 07~09시 피크 전날 22시에 고갈 예상 대여소 목록 자동 생성
- **재배치 동선 최적화**: 주변 대여소의 여유분·예측 수요를 동시에 분석해 공급원 자동 추천 → 재배치 트럭 동선 사전 최적화
