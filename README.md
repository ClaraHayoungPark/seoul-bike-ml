...
아래 첫 번째 그림은 README의 핵심 결론인 `무슨 이상이 많았는지`와 `언제 집중됐는지`를 함께 보여줍니다. 두 번째 히트맵은 그 패턴이 **어느 대여소·날짜에 반복적으로 나타났는지**를 보조적으로 확인하는 용도입니다.

![STL 이상 패턴 분류 및 시간대별 급증·급감 분포](reports/figures_readme/stl_pattern_calendar.png)

![상위 30개 대여소 기준 STL 이상 발생 히트맵](reports/figures_readme/stl_station_heatmap.png)
